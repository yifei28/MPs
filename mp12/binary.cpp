...
/***
In the context of implementing a binary search tree (BST) using C++, 
we developed a comprehensive structure consisting of two primary classes: 
the BinaryTree and Node classes.
***/

/***
The Node class serves as the fundamental building block of our tree, 
encapsulating both the key and data elements that the tree manages, 
alongside pointers to left and right child nodes. 
In addition, it supports multiple datatype for key and data as this is a template class.
***/

/***
The BinaryTree class manages these nodes, providing functionalities for inserting new nodes, 
performing different types of tree traversals (pre-order, in-order, post-order), 
and locating specific nodes based on key values. 
This structure not only organizes data efficiently but also supports efficient data retrieval operations
***/
...
{
    // base case: if current node is NULL, insert and return
    if (parent_node == NULL)
    {
        root = data;
        return;
    ...
    // recursive case
    if (data->getKey() < parent_node->getKey()) // if smaller, go left
    {
    ...
    else if (data->getKey() > parent_node->getKey()) // if greater, go right
    {
...
{
    if (node == NULL) // when reach the end
    { 
        return node;
    }
    else if (node->getKey() > key) // when key is smaller, go left
    { 
        node->left = delete_node(node->left, key);
    }
    else if (node->getKey() < key) // when key is greater, go right
    {
        node->right = delete_node(node->right, key);
    }
    else // when key node is found
    { 
        ...
        else if (node->right == NULL) // if no right child
        { 
        ...
        { 
            Node<T, Y> *temp = node->right;
            while (temp->left != NULL) // search for the smallest key that is still greater than the target node
            { 
                temp = temp->left;
            }
            node->setData(temp->getData()); // replace the target node
            node->setKey(temp->getKey());
            node->right = delete_node(node->right, temp->getKey()); // delete the excessive key that replaces the target node at original position
        }
    }
   return node;
}
...
{
    // base case1: node is empty, or has no children
    if ((node == NULL) || (node->left == NULL && node->right == NULL))
    {
        return NULL;
    }
    // base case2: key found at left children or right children
    ...
    // recursive case:
    if (node->getKey() < key) // if key is greater than that of node, go right
    {
...
Node<T, Y> *BinaryTree<T, Y>::find_node(Node<T, Y> *node, T key)
{
    ...
    else // if key is smaller than that of node, go left
    {
